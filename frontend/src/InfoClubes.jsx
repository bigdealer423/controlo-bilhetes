// InfoClubes.jsx LIMPO E FUNCIONAL
      ...
      <table className="min-w-full border border-gray-300">
        ...
        <tbody>
          {clubes.map((clube, index) => (
            <tr key={clube.id} className="border-b hover:bg-gray-50">
              ...
              {['estadio', 'capacidade', 'site', 'locaisVenda'].map((campo) => (
                <td key={campo} className="p-2 border text-center">
                  {editIndex === index ? (
                    <input type="text" value={editClube[campo]} onChange={e => setEditClube({ ...editClube, [campo]: e.target.value })} className="border p-1 rounded w-full" />
              ...
              <td className="p-2 border text-center flex gap-2 justify-center">
                {editIndex === index ? (
                  <button onClick={handleSaveEdit} className="text-green-600"><FiPlus /></button>
                ) : (
